...
import {useCalendar} from '../../datePicker/DatePicker';
const Header = ({ changeMonth = () => null }) => {
  ...
  const [{ lastDate, shownAnimation, hiddenAnimation }, changeMonthAnimation] = utils.useMonthAnimation(
  ...
  const nextDisable = disableDateChange || (maximumDate && utils.checkArrowMonthDisabled(mainState.activeDate, false));
      ...
      <View style={style.monthYearContainer}>
        ...
        >
          ...
          {mode === 'datepicker' && (
            ...
            >
              <Text style={style.headerText}>{utils.toPersianNumber(utils.getTime(mainState.activeDate))}</Text>
            </TouchableOpacity>
          )}
        </Animated.View>
        <Animated.View
